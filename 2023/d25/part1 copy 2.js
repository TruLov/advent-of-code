...
export default function solve(input) {
    ...
    const eig = numeric.eig(laplace_matrix);
    const eigenvalues = eig.lambda.x;
    const eigenvectors = eig.E.x;
    const secondSmallestIndex = eigenvalues
        .slice(1)
        .reduce(
            (minIndex, value, currentIndex) =>
                value < eigenvalues[minIndex] ? currentIndex + 1 : minIndex,
            1
        );
    const fiedlerVector = eigenvectors.map((row) => row[secondSmallestIndex]);
    ...
    const group_size = fiedlerVector.filter((f) => f > 0).length;
